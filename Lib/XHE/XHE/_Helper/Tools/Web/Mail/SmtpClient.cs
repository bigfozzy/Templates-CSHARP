...
#region Methods
        ...
        /// отправить мыло
        /// </summary>
        /// <param name="mailMessage">письмо</param>
        /// <returns>результат</returns>
        public bool SendMailNew(MailMessage mailMessage)
        {
            try
            {
                ...
                for (int i = 0; i < commands.Length; i++)
                {
                    ...
                    else
                    {
                        if (i == 0)
                        {
                            ...
                            {
                                this.errorMessage = "Did not get " + responses[0] + " reply from server for command " +
                                                    commands[0] + ".\r\nServer said: " + responseText;
                                this.tcp_Socket.Close();
                                throw new SmtpClientException(this.errorMessage);
                            }
                            else
                            {
                                ...
                                if (this.WaitForResponse("220", ref responseText, true) == false)
                                {
                                    this.errorMessage = "Did not get " + "220" + " reply from server for command " +
                                                        "STARTTLS" + ".\r\nServer said: " + responseText;
                                    this.tcp_Socket.Close();
                                    throw new SmtpClientException(this.errorMessage);
                                }
                                else
                                {
                                    ...
                                    if (this.secureStream.IsAuthenticated)
                                    {
                                        ...
                                        {
                                            this.errorMessage = "Did not get " + responses[0] +
                                                                " reply from server for command " + commands[0] +
                                                                ".\r\nServer said: " + responseText;
                                            this.tcp_Socket.Close();
                                            throw new SmtpClientException(this.errorMessage);
                                        }
                                        ...
                                        {
                                            if (
                                                responseText.IndexOf("Auth Login", StringComparison.OrdinalIgnoreCase) >=
                                                0)
                                            {
                                                message = "Auth Login" + "\r\n";
                                                this.SendDataThroughSecureStream(message);
                                                ...
                                                {
                                                    this.errorMessage =
                                                        "Did not get 334 reply from server for AUTH LOGIN" +
                                                        ".\r\nServer said: " + responseText;
                                                    this.tcp_Socket.Close();
                                                    throw new SmtpClientException(this.errorMessage);
                                                }
                                                else
                                                {
                                                    ...
                                                    {
                                                        this.errorMessage =
                                                            "Did not get 334 reply from server for UserName Confirmation" +
                                                            ".\r\nServer said: " + responseText;
                                                        this.tcp_Socket.Close();
                                                        throw new SmtpClientException(this.errorMessage);
                                                    }
                                                    else
                                                    {
                                                        ...
                                                        {
                                                            this.errorMessage =
                                                                "Did not get 235 reply from server for Password Confirmation" +
                                                                ".\r\nServer said: " + responseText;
                                                            this.tcp_Socket.Close();
                                                            throw new SmtpClientException(this.errorMessage);
                                                        }
                        ...
                        else if (i == 4)
                        {
                            ...
                            {
                                this.errorMessage = "Did not get " + responses[i] + " reply from server for command " +
                                                    commands[i] + ".\r\nServer said: " + responseText;
                                this.tcp_Socket.Close();
                                throw new SmtpClientException(this.errorMessage);
                            }
            ...
            {
                XHE._Helper.Tools.GUI.ShowMessage.ShowWarningMessage(ex.Message);

            }
            return true;
        }
        /// <summary>
        /// рассоединение smtp соединения
        /// </summary>
        /// <returns></returns>
